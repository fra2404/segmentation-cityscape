...
\pagestyle{fancy}
\fancyhead[C]{\textcolor{headergray}{Project report for Artificial Intelligence --- BUAA (Project 2: Semantic Segmentation)}}
...
\begin{abstract}
This report describes the implementation and evaluation of a semantic segmentation pipeline on the Cityscapes dataset using a DeepLabV3+ architecture. The project includes an end-to-end training script configurable from the command line, a robust checkpointing system enabling interruption and exact resumption of training, and utilities to generate predictions compliant with the official Cityscapes submission format. Development and early experiments were conducted on macOS (CPU/MPS) for rapid iteration, while full training was executed on Google Colab using an NVIDIA T4 GPU to significantly reduce training time.
\end{abstract}
...
\item A custom raw-file loader (\texttt{CityscapesRawDataset}) designed to mirror a notebook-like pipeline and to support Albumentations transforms jointly on image and mask.
\end{itemize}
...
Early development and sanity checks were executed on macOS using CPU/MPS to speed up iteration and debugging. Full training was then moved to Google Colab and executed on an NVIDIA T4 GPU, which substantially reduces training time compared to CPU/MPS.
...
Table~\ref{tab:checkpoint_summary} summarizes representative checkpoints. For readability, the table is split into two columns.
...
\end{lstlisting}
This will save an image with the predicted segmentation overlaid on the original. By repeating this for a few diverse samples (e.g., scenes with traffic, pedestrians, or different lighting), you can qualitatively inspect the model's performance. Example overlays are included in the appendix.
...
Despite the solid performance, the achieved mIoU is lower than state-of-the-art results on Cityscapes, mainly due to limited training epochs, the use of a ResNet-50 backbone, and the absence of large-scale pretraining beyond ImageNet. Future work could therefore focus on longer training schedules, stronger backbones (or larger DeepLab variants), and enhanced pretraining or semi-supervised strategies.
...
This project was developed as part of the Artificial Intelligence course requirements at Beihang University (BUAA).
...
\begin{itemize}
  ...
  \item One external photo (test.webp) downloaded from the web
\end{itemize}
